#!/usr/bin/env python
"""
	customising by hrchoi <dime9880@gmail.com> 
	
	Original scalp can analyse only apache web log. But I need to analysis various format web log. So, I customise scalp that it be able to anlysis more than yet.  
...
__version__     = "0.4.revision" # modified
__release__     = __application__ + '/' + __version__
__author__      = "Romain Gaucher, custom by hrchoi" # modified
...
apache_c_reg = re.compile(r'^(.+)-(.*)\[(.+)[-|+](\d+)\] "([A-Z]+)?(.+) HTTP/\d.\d" (\d+)(\s[\d]+)?(\s"(.+)" )?(.*)$') # modified 
webtob_c_reg = re.compile(r'^(.+) (\[.*\]) "([A-Z]+)?(.*)HTTP\/\d.\d" (.+) (.+) (.+)') # added 
iis_c_reg = re.compile(r'') # iis use no reg 
# need to more than format
...
def analyzer(data):
	exp_line, regs, array, preferences, org_line, checker = data[0],data[1],data[2],data[3],data[4], data[5] # modified 
	done = []
		...
		if attack_type in regs:
			...
			for _hash in regs[attack_type]:
				if _hash not in done:
					...
					attack = table[_hash]
					if checker == 'apache': # added 
						cur_line = exp_line[5] # added
					elif checker == 'webtob': # added 
						cur_line = exp_line[3] # added 
					elif checker == 'iis': # added  
						cur_line = exp_line[4] # added  
...
def scalper(access, filters, format, preferences = [], output = "text"): # modified
	global table
	...
	if format == 'apache': 
		...
		with open(access) as log_file:
			for line in log_file:
				...
				if sample and lines not in sampled_lines:
					continue
				if apache_c_reg.match(line):
					out = apache_c_reg.search(line)
					ip = out.group(1)
					name  = out.group(2)
					date = out.group(3)
					ext  = out.group(4)
					...
					if len(url) > 1 and method in ('GET','POST','HEAD','PUT','PUSH','OPTIONS'):
						analyzer([(ip,name,date,ext,method,url,response,byte,referrer,agent),regs,flag, preferences, line, apc_chk]) # modified 
		...
		with open(access) as log_file:
			for line in log_file:
				...
				if sample and lines not in sampled_lines:
					continue
				if webtob_c_reg.match(line):
					out = webtob_c_reg.search(line)
					ip = out.group(1) # modified 
					# name  = out.group(2) # removed 
					date = out.group(2) # modified 
					# ext  = out.group(4) # removed 
					method = out.group(3) # modified 
					url = out.group(4) # modified 
					response = out.group(5) # modified 
					byte = out.group(6) # modified 
					rtime = out.group(7) # modified 
					# referrer = out.group(9) # removed 
					# agent = out.group(10) # removed 

					#if not correct_period(date, preferences['period']): # not yet  
					#	continue # not yet  
					loc += 1
					if len(url) > 1 and method in ('GET','POST','HEAD','PUT','PUSH','OPTIONS'):
						# analyzer([(ip,name,date,ext,method,url,response,byte,referrer,agent),regs,flag, preferences, line]) # removed 
						analyzer([(ip,date,method,url,response,byte,rtime),regs,flag, preferences, line, wtb_chk]) # modified 
				elif preferences['except']:
					diff.append(line)
		...
		with open(access) as log_file:
			for line in log_file:
				...
				if line != '': # modified
					if line[0] != '#': # modified
						try: # modified
							out = line.split(' ') # modified
							tmp = out[0] + ' ' + out[1] # modified
							out.insert(0, tmp) # modified
							del out[1] # modified
							del out[1] # modified
							date = out[0] # modified
							ip = out[1] # modified 
							method = out[2] # modified
							tmp = out[3] + '?' + out[4] # modified
							out.insert(3, tmp) # modified
							del out[4] # modified
							del out[4] # modified
							url = out[3] # modified
							name = out[5] # modified
							agent = out[7] # modified
							response = out[8] # modified
							rtime = out[11] # modified 
							# byte = # modified
							# referrer = # modified
							...
							if len(url) > 1 and method in ('GET','POST','HEAD','PUT','PUSH','OPTIONS'):
								# analyzer([(ip,name,date,ext,method,url,response,byte,referrer,agent),regs,flag, preferences, line]) # removed 
								analyzer([(ip,name,date,method,url,response,rtime,agent),regs,flag, preferences, line, iis_chk]) # modified 
						except (IndexError) as e: # modified
							print e # modified
				elif preferences['except']: # modified
					diff.append(line)
	...
	if n > 0:
		...
		if 'html' in preferences['output']:
			...
			generate_text_file(flag, short_name, filters, s_m_i, preferences['odir']) # modified 
...
def generate_text_file(flag, access, filters, s_m_i, odir): # modified 
	...
	try:
		...
		out.write("Loaded XML file: %s\n\n" % filters) # modified 
		out.write("Scalp results:") # modified 
		out.write("\tProcessed %d lines over %d\n" % (s_m_i[0],s_m_i[1])) # modified 
		out.write("\tFound %d attack patterns in %f s\n\n" % (s_m_i[2],s_m_i[3])) # modified 
...
def help():
	print "'Custom' Scalp not only the apache log but also the various web log! by Romain Gaucher - http://rgaucher.info, customising by hrchoi - dime9880@gmail.com" # modified
	print "usage:  ./scalp.py [--log|-l log_file] [--format|-F log_format] [--filters|-f filter_file] [--period time-frame] [OPTIONS] [--attack a1,a2,..,an]" # modified 
	print "                   [--sample|-s 4.2]"
	print "   --log       |-l:  the log file './access_log' by default" # modified
	print "   --format    |-F:  the log file format 'apache' by default" # modified
	print "                     list: apache, webtob, iis, ..." # modified, need to add more format  
	...
	if argc < 2 or sys.argv[1] == "--help":
		...
		for i in range(argc):
			...
			if i < argc:
				...
				elif s in ("--format", "-F"): # modified
					format = argv[i+1] # modified 
				elif s in ("--attack", "-a"):
		...
		if not os.path.isdir(preferences['odir']):
			...
			try:
				...
				os.mkdir(preferences['odir'])
		scalper(access, filters, format, preferences) # modified
